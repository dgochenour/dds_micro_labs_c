...
DDS_Long sleep_time, DDS_Long count)
{
    ...
    *DDS_StringSeq_get_reference(&dp_qos.discovery.initial_peers,0) = DDS_String_dup(peer);
    ...
    /* LAB #4 -- change the type name to match what Admin Console expects when
     * using modules ("::" instead of "_")
     */
    sprintf(application->type_name, "myModule::msgType");
    retcode = DDS_DomainParticipant_register_type(application->participant,
    application->type_name,
    myModule_msgTypeTypePlugin_get());
    ...
    /* LAB #5 -- update Topic name to use the constant defined in the IDL */
    sprintf(application->topic_name, myModule_my_topic_name);
    application->topic =
